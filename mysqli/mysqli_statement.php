...
if (isset($_GET['search'])) {
    try {
        ...
        if (!$stmt->prepare($sql)) {
            ...
            $make = '%' . $db->real_escape_string($_GET['make']) . '%';
            $stmt->execute();
            //$stmt->bind_result($make, $yearmade, $mileage, $price, $description);
...
if (isset($_GET['search'])) {
    ...
    if (!$numrows) {
        ...
        <table>
            ...
            <?php
            /*
             * fetch() for binding result
             * fetch_row() next row as indexed array
             * fetch_assoc() next row as associative array
             * fetch_array() offers choice of array types
             * fetch_all() create array containing all rows (choice by passing variable MYSQLI_BOTH, MYSQLI_ASSOC)
             * fetch_object() next row as object
             */
